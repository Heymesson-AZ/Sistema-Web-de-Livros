    ...
    <div class="container py-4">
        ...
        <div
            ...
            <div>
                <h1 class="h3 fw-bold text-dark mb-1">
                    ...
                    Catálogo da Minha Loja
                </h1>
                <p class="text-muted small mb-0">
                    Gerencie os livros publicados por <strong>{{ $vendedor->nome_fantasia }}</strong> na Universo de
                    Papel.
                </p>
            </div>
            <a href="{{ route('vendedor.livros.create') }}"
                class="btn btn-success px-3 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
                <i class="bi bi-plus-circle-fill"></i>
                <span>Publicar Novo Livro</span>
            </a>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4">
            <div class="card-body p-4">
                <form action="{{ route('vendedor.livros.index') }}" method="GET" class="row g-3">
                    ...
                    <div class="col-12 col-md-1 d-flex align-items-end gap-1">
                        <button type="submit" class="btn btn-success w-100" title="Filtrar">
                            <i class="bi bi-funnel-fill"></i>
                        </button>
                        @if (request()->hasAny(['busca', 'genero_id', 'status_estoque', 'ordem']))
                            <a href="{{ route('vendedor.livros.index') }}" class="btn btn-outline-secondary"
                    ...
                    <tbody>
                        @forelse($livros as $livro)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('vendedor.livros.show', $livro) }}"
                                ...
                                <td class="text-end pe-4">
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        <a href="{{ route('vendedor.livros.edit', $livro) }}"
                                            class="btn btn-outline-primary" title="Editar">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button type="button" class="btn btn-outline-danger" title="Excluir"
                                            data-bs-toggle="modal"
                                            data-bs-target="#deleteModalVendedor{{ $livro->id }}">
                                            <i class="bi bi-trash"></i>
                                        </button>
                                    </div>

                                    <!-- MODAL DE EXCLUSÃO -->
                                    <div class="modal fade" id="deleteModalVendedor{{ $livro->id }}"
                                        tabindex="-1" aria-labelledby="deleteModalLabel{{ $livro->id }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content rounded-4 border-0 shadow">
                                                ...
                                                <div class="modal-body text-start py-3">
                                                    <p class="mb-2">Deseja remover
                                                        <strong>"{{ $livro->titulo }}"</strong> da sua loja?</p>
                                                    <p class="small text-muted mb-0">Esta ação remove o livro do
                                                        catálogo público com segurança (Soft Delete), sem afetar pedidos
                                                        anteriores.</p>
                                                </div>
                                                <div class="modal-footer border-0 pt-0">
                                                    <button type="button" class="btn btn-light rounded-pill px-3"
                                                        data-bs-dismiss="modal">Cancelar</button>
                                                    <form action="{{ route('vendedor.livros.destroy', $livro) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    ...
                                    Nenhum livro cadastrado na sua loja ainda.
                                    <div class="mt-2">
                                        <a href="{{ route('vendedor.livros.create') }}"

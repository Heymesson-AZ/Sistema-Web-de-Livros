<x-layouts.principal>
    <div class="container py-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
            ...
            <a href="{{ route('admin.clientes.create') }}" class="btn btn-primary px-3 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
            ...
            <form method="GET" action="{{ route('admin.clientes.index') }}" class="row g-2 align-items-center">
                <div class="col-12 col-md-9">
                    <div class="input-group">
                        ...
                        <input type="text" name="busca" class="form-control border-start-0 ps-0" placeholder="Buscar por nome, e-mail, CPF ou celular..." value="{{ request('busca') }}">
                    </div>
                </div>
                ...
                <div class="col-12 col-md-3 d-flex gap-2">
                    ...
                    @if(request('busca'))
                        <a href="{{ route('admin.clientes.index') }}" class="btn btn-outline-secondary" title="Limpar Busca">
                            ...
                            <tr>
                                <td class="ps-4 py-3">
                                    <div class="d-flex align-items-center gap-3">
                                        <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm"
                                             style="width: 44px; height: 44px; background: #e0e7ff; color: #4338ca; font-size: 1.1rem;">
                                            {{ strtoupper(substr($cliente->user?->name ?? 'C', 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="fw-bold text-dark">{{ $cliente->user?->name ?? 'Sem Nome' }}</div>
                                ...
                                <td class="py-3">
                                    <div class="text-secondary small">
                                        <i class="bi bi-phone me-1"></i>{{ $cliente->celular_contato ?? 'Não informado' }}
                                    </div>
                                </td>
                                ...
                                <td class="py-3 text-center">
                                    <span class="badge bg-light text-dark border px-2 py-1 me-1" title="Total de Pedidos">
                                        <i class="bi bi-bag-check me-1 text-success"></i> {{ $cliente->pedidos_count ?? 0 }}
                                    </span>
                                    <span class="badge bg-light text-dark border px-2 py-1" title="Avaliações Feitas">
                                        <i class="bi bi-star me-1 text-warning"></i> {{ $cliente->avaliacoes_count ?? 0 }}
                                    </span>
                                </td>

                                <td class="pe-4 py-3 text-end">
                                    <div class="btn-group gap-1">
                                        <a href="{{ route('admin.clientes.show', $cliente) }}" class="btn btn-sm btn-outline-secondary rounded-3" title="Visualizar Detalhes">
                                            <i class="bi bi-eye"></i>
                                        </a>

                                        <a href="{{ route('admin.clientes.edit', $cliente) }}" class="btn btn-sm btn-outline-primary rounded-3" title="Editar Cliente">
                                            <i class="bi bi-pencil"></i>
                                        </a>

                                        <button type="button" class="btn btn-sm btn-outline-danger rounded-3" data-bs-toggle="modal" data-bs-target="#deleteClienteModal{{ $cliente->id }}" title="Excluir Cliente">
                                            ...
                                            <div class="modal-content border-0 shadow-lg rounded-4">
                                                ...
                                                <div class="modal-body py-3">
                                                    <p class="text-secondary mb-2">
                                                        Tem certeza que deseja excluir o cliente <strong>{{ $cliente->user?->name }}</strong> (CPF: {{ $cliente->cpf }})?
                                                    </p>
                                                    <p class="text-danger small mb-0 fw-semibold">
                                                        Atenção: A conta de acesso do cliente também será excluída permanentemente.
                                                    </p>
                                                </div>
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <p class="small mb-3">Tente ajustar seus termos de busca ou cadastrar um novo cliente.</p>
                                        @if(request('busca'))
                                            <a href="{{ route('admin.clientes.index') }}" class="btn btn-sm btn-outline-primary rounded-3">
                                                Limpar Busca
                                            </a>
                                        @endif
                                    </div>

        ...
        <div class="mb-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active text-primary fw-semibold" aria-current="page">{{ $cliente->user?->name }}</li>
                </ol>
            </nav>
            ...
            <div class="col-12 col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 p-4 text-center h-100">
                    <div class="rounded-circle d-inline-flex align-items-center justify-content-center fw-bold shadow-sm mx-auto mb-3"
                         style="width: 80px; height: 80px; background: #e0e7ff; color: #4338ca; font-size: 2rem;">
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 mb-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <span class="fw-semibold text-dark fs-6">{{ $cliente->celular_contato ?? 'Não informado' }}</span>
                        </div>
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <span class="fw-semibold text-dark fs-6">{{ $cliente->created_at?->format('d/m/Y \à\s H:i') ?? '-' }}</span>
                        </div>

                        <div class="col-12 col-sm-6">
                            <span class="text-muted small d-block">Última Atualização</span>
                            <span class="fw-semibold text-dark fs-6">{{ $cliente->updated_at?->format('d/m/Y \à\s H:i') ?? '-' }}</span>
                        </div>
                    </div>
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            @if ($cliente->user?->email_verified_at)
                                <span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-2 py-1 small">
                                    <i class="bi bi-check-circle-fill me-1"></i> Verificado em {{ $cliente->user->email_verified_at->format('d/m/Y') }}
                                </span>
                            @else

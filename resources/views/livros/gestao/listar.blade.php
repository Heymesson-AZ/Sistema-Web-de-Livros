    ...
    <div class="container py-4">
        ...
        <nav aria-label="breadcrumb" class="mb-3">
            <ol class="breadcrumb mb-0">
                ...
                @if ($ehAdmin)
                    <li class="breadcrumb-item active text-primary fw-semibold" aria-current="page">Moderação do Catálogo
                    </li>
                @else
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
            <div>
                ...
                <p class="text-muted small mb-0">
                    ...
                    @else
                        Gerencie os títulos publicados pela sua livraria
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-3 bg-white h-100">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <span class="h4 fw-bold text-primary mb-0">R$
                                    {{ number_format($valorTotalEstoque, 2, ',', '.') }}</span>
                            </div>
            ...
            <div class="card-body p-3 p-md-4">
                <form action="{{ route($rotaPrefix . '.index') }}" method="GET">
                    <div class="row g-2">
                        ...
                        <div class="col-12 col-md-{{ $ehAdmin ? '3' : '4' }}">
                            <div class="input-group">
                                ...
                                <input type="text" name="busca" class="form-control border-start-0"
                                    placeholder="Buscar título, ISBN ou autor..." value="{{ request('busca') }}"
                        ...
                        <div class="col-6 col-md-2">
                            <select name="genero_id" class="form-select">
                                ...
                                @foreach ($generos as $gen)
                                    <option value="{{ $gen->id }}"
                                        {{ request('genero_id') == $gen->id ? 'selected' : '' }}>
                                        {{ $gen->nome }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="editora_id" class="form-select">
                                ...
                                @foreach ($editoras as $edit)
                                    <option value="{{ $edit->id }}"
                                        {{ request('editora_id') == $edit->id ? 'selected' : '' }}>
                                        {{ $edit->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if ($ehAdmin)
                            ...
                            <div class="col-6 col-md-2">
                                <select name="status_moderacao" class="form-select">
                                    ...
                                    <option value="ativo"
                                        {{ request('status_moderacao') === 'ativo' ? 'selected' : '' }}>Ativo</option>
                                    <option value="sob_analise"
                                        {{ request('status_moderacao') === 'sob_analise' ? 'selected' : '' }}>Sob
                                        Análise</option>
                                    <option value="bloqueado_temporariamente"
                                        {{ request('status_moderacao') === 'bloqueado_temporariamente' ? 'selected' : '' }}>
                                        Bloqueado</option>
                                    <option value="banido"
                                        {{ request('status_moderacao') === 'banido' ? 'selected' : '' }}>Banido
                                    </option>
                                </select>
                            </div>
                        @else
                            ...
                            <div class="col-6 col-md-2">
                                <select name="status_estoque" class="form-select">
                                    ...
                                    <option value="com_estoque"
                                        {{ request('status_estoque') === 'com_estoque' ? 'selected' : '' }}>Disponível
                                    </option>
                                    <option value="sem_estoque"
                                        {{ request('status_estoque') === 'sem_estoque' ? 'selected' : '' }}>Zerado
                                    </option>
                                </select>
                            </div>
                        @endif
                        ...
                        <!-- Botão Filtrar -->
                        <div class="col-12 col-md-{{ $ehAdmin ? '1' : '2' }} d-flex gap-1">
                            <button type="submit" class="btn-search-submit w-100 px-2" title="Filtrar Catálogo"
                                aria-label="Filtrar">
                            ...
                            @if (request()->hasAny([
                                ...
                                <a href="{{ route($rotaPrefix . '.index') }}" class="btn-search-clear"
                    ...
                    <tbody>
                        @forelse($livros as $livro)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="d-block text-dark small fw-semibold">{{ $livro->autor->nome ?? 'N/A' }}</span>
                                    <span
                                        class="badge bg-light text-secondary border">{{ $livro->genero->nome ?? 'N/A' }}</span>
                                </td>
                                @if ($ehAdmin)
                                    <td>
                                        <span
                                            class="small fw-semibold text-dark">{{ $livro->vendedor->nome_fantasia ?? 'N/A' }}</span>
                                    </td>
                                @endif
                                <td>
                                    <strong class="text-success">R$
                                        {{ number_format($livro->preco, 2, ',', '.') }}</strong>
                                </td>
                                <td>
                                ...
                                <td class="text-end pe-4">
                                    <div class="actions-group justify-content-end">
                                        ...
                                        <a href="{{ route($rotaPrefix . '.edit', $livro) }}"
                                            class="btn-action btn-action-edit" title="Editar Livro"
                                            aria-label="Editar">
                                        ...
                                        <!-- BOTÃO VER DETALHES -->
                                        <a href="{{ route($rotaPrefix . '.show', $livro) }}"
                                            class="btn-action btn-action-view" title="Inspecionar Dados e Histórico"
                                    ...
                                    @if ($ehAdmin && Auth::user()->podeModerarLivros())
                                        ...
                                        <div class="modal fade" id="modalModeracao{{ $livro->id }}"
                                                    ...
                                                    <form action="{{ route('admin.livros.status', $livro) }}"
                                                        ...
                                                        <div class="modal-body py-3">
                                                            <p class="text-muted small mb-3">
                                                                Selecione a ação disciplinar ou liberação para este
                                                                exemplar. Livros não ativos ficam ocultos da loja
                                                                pública.
                                                            </p>
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <select name="status_moderacao"
                                                                    ...
                                                                    <option value="sob_analise"
                                                                        ...
                                                                        Sob Análise (Dados suspeitos ou pendência
                                                                        documental)
                                                                    </option>
                                                                    <option value="bloqueado_temporariamente"
                                                            ...
                                                            @if ($livro->moderado_em)
                                                                <div
                                                                    class="p-2.5 rounded-3 bg-light border small text-muted">
                                                                    <i class="bi bi-clock-history me-1"></i>
                                                                    Última moderação em
                                                                    <strong>{{ $livro->moderado_em->format('d/m/Y H:i') }}</strong>
                                                                    por
                                                                    <strong>{{ $livro->moderador?->name ?? 'Administrador' }}</strong>.
                                                                </div>
                                                            @endif
                                                        </div>
